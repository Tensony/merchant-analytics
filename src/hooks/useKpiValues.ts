...
function sumRevenue(data: DailyDataPoint[]): number {
  return data.reduce((acc, d) => acc + d.revenue, 0);
}

function sumOrders(data: DailyDataPoint[]): number {
  return data.reduce((acc, d) => acc + d.orders, 0);
}

function avgAOV(data: DailyDataPoint[]): number {
  const total  = sumRevenue(data);
  const orders = sumOrders(data);
  return orders > 0 ? total / orders : 0;
}

function avgChurn(data: DailyDataPoint[]): number {
